			...
			AS 
				BEGIN
					IF EXISTS(SELECT 1 FROM deleted) -- EL 1 SIGNIFICA UN TRUE, SI EXISTE UN REGISTRO EN LA TABLA TEMPORAL DELETED
						-- SI NO HAY NINGUN DELETED ENTONCES SE VA AL ELSE
						BEGIN
							...
							SELECT D.DNI, D.APPATERNO, D.APMATERNO, D.NOMBRES, D.EDAD, D.ESTADO, GETDATE(),
								...
								ELSE
									'MODIFICADO'
								END) AS VACCION 
							FROM deleted D JOIN inserted I ON D.DNI=I.DNI -- DELETED ES UNA TABLA TEMPORAL QUE SE REGISTRA CUANDO HAY UN UPDATE O DELETED
						END
					ELSE
						BEGIN
							INSERT INTO HISTORICO_ALUMNO
							...
							FROM inserted I	   --INSERTED ES UNA TABLA TEMPORAL
						END
				END
	GO
